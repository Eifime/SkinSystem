<?php
   ...
   *
   * format - The format in which the image is to be rendered. "png", "svg", "base64", "raw", "signed". (png by default)
   *
  ...
  if(__FILE__ === $_SERVER['SCRIPT_FILENAME']) {
    // only adjust the error reporting if we aren't an included file
  ...
  if(basename(__FILE__) === basename($_SERVER["SCRIPT_FILENAME"]) && ((grabGetValue('user') !== false) || (grabGetValue('mojang') !== false))) {
    ...
    $rendpr = array(grabGetValue('mojang'),
                    grabGetValue('user'),
                    grabGetValue('vr'),
                    grabGetValue('hr'),
                    grabGetValue('hrh'),
                    ...
                    grabGetValue('aa'),
                    grabGetValue('layers'));
    $player = new render3DPlayer(...$rendpr);
    unset($rendpr[0]); unset($rendpr[1]); $rendpr = serialize($rendpr); // remove user-specific render params and serialize
    $res = $player->getSkinFile();

    if (in_array(grabGetValue('format'), ['raw', 'signed', 'base64'])) { $cfile = $res['raw']; }
    else { // cache system for "rendered" skins
      $cfile = DIR_ROOT.'/'.$config['cache_dir'].$res['mj256'].'-'.hash("md5", $rendpr);
      if (!is_file($cfile) or curl($cfile) === '') { $player->get3DRender($res['raw'], $cfile); }
    }
    // respond with 304 if applicable ($cfile must be set)
    if ((isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) && strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) > filemtime($cfile)) ||
    (isset($_SERVER['HTTP_IF_UNMODIFIED_SINCE']) && strtotime($_SERVER['HTTP_IF_UNMODIFIED_SINCE']) < filemtime($cfile))) {
      header('HTTP/1.0 304 Not Modified');
      die();
    } elseif ($_SERVER['REQUEST_METHOD'] === 'GET') {
      if(grabGetValue('format') === 'signed'){
        header('Content-Type: application/json');
        echo json_encode(array_combine(['value', 'signature'], $res['sr']));
      } else {
        if ($_GET['dl'] === 'true') {
          header('Content-Disposition: attachment; filename='.grabGetValue('user').'-'.hash('adler32', $rendpr.$res['raw']).'.png');}
  ...
  class render3DPlayer { 
    ...
    public function getSkinFile() {
      ...
      if ($this->mojang!=null) {
        ...
        $skinName = query('sr', "SELECT {$tb[2]} FROM {$tb[0]} WHERE {$tb[1]} = ?", [$this->playerName])->fetch(PDO::FETCH_ASSOC)[$tb[2]];
        $tb = get_table_names('sr', 0); // get names (Skins)
        $skinLookup = array_values(query('sr', "SELECT {$tb[2]}, {$tb[3]} FROM {$tb[0]} WHERE {$tb[1]} = ?", [$skinName])->fetch(PDO::FETCH_ASSOC));
        $skinURL = json_decode(base64_decode($skinLookup[0]), true)['textures']['SKIN']['url'];
        if (!is_string($skinURL)) { // if cannot get skin from skinsrestorer, get skin from mojang.
          $mjdfskfl = DIR_ROOT.'/'.$config['cache_dir'].'mojang_skin-'.strtolower($this->playerName);
          if (file_exists($mjdfskfl)) { // fetch cached mojang skin
            $skinURL = curl($mjdfskfl);
          } else { // fetch and cache mojang version of skin
            $pf = json_decode(curl('https://api.mojang.com/users/profiles/minecraft/'.strtolower($this->playerName)), true);
            if(is_array($pf) and array_key_exists("id", $pf)) { 
              $uuid = $pf["id"];
              $ca = json_decode(curl('https://sessionserver.mojang.com/session/minecraft/profile/'.$uuid), true);
              if(is_array($ca) and array_key_exists("properties", $ca)) { 
                foreach($ca["properties"] as $element) {
                  if(array_key_exists("name", $element) && $element["name"] == "textures") {
                    $skinArray = json_decode(base64_decode($element["value"]), true);
                    if(array_key_exists("textures", $skinArray) and array_key_exists("SKIN", $skinArray["textures"])){
                      $skinURL = $skinArray["textures"]["SKIN"]["url"];
                      file_put_contents($mjdfskfl, $skinURL);
                      break;
      }}}}}}}}
      if (strlen($skinURL) < 1) { // get default skin of player, alex or steve.
        if (!isset($uuid)) { $uuid = md5("OfflinePlayer:".$this->playerName); } // calculate offline-mode uuid https://gist.github.com/vojtamaniak/03ccb8b4df97562981f2
        $uuid = strtolower(str_replace("-","",trim($uuid)));
        for( $i=0; $i<4; $i++){ $sub[$i] = intval(substr($uuid,$i*8,8), 16); } // calc java hashcode then if steve/alex
        $char = ['steve', 'alex'][(($sub[0]^$sub[1])^($sub[2]^$sub[3]))%2]; // https://github.com/sh4ni/Minecraft-UUID-Tools/blob/d5bae15835c2a378812dd8cc91666a6a013a4168/alexsteve.php#L5
        $skinURL = 'https://raw.githubusercontent.com/InventivetalentDev/minecraft-assets/master/assets/minecraft/textures/entity/'.$char.'.png';
      }
      if (!$mj256) { preg_match('/[^\/\\\]+$/', $skinURL, $mj256); }
      return ['raw'=>cacheGrab($skinURL, $mj256[0], DIR_ROOT.'/', false, ['sha256', $mj256[0]]), 'mj256'=>$mj256[0], 'sr'=>$skinLookup];
    }
